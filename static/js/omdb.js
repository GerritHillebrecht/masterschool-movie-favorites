const OMDB_API_KEY="6999413f";
const API_URI = `http://www.omdbapi.com/?apikey=${OMDB_API_KEY}&`
const search_container = document.getElementById("search_results")

let currentAbortController;

const searchInput = document.getElementById("default-search");
searchInput.addEventListener("keyup", async ({target: {value}}) => {
    if (value.length > 2) {
        ...
        try {
            ...
            search_container.appendChild(create_loader())

            const movie = await fetch_movies(value, signal)

            if (movie.name) {
                search_container.innerText = ""
                search_container.appendChild(create_search_result(movie))
            } else {
                const not_found_message = document.createElement("p");
                not_found_message.className = "p-4";
                not_found_message.textContent = `No movie found with title ${value}`

                search_container.innerText = ""
                search_container.appendChild(not_found_message)
            }
        } catch(error) {
...
function create_search_result(movie) {
    ...
    movie_container.className = "shadow p-4 bg-white/80 rounded-lg grid grid-cols-[1fr,2fr] gap-4";
    ...
    add_movie_button.className = "pointer-events-auto mt-2 px-3 py-2 text-xs font-medium text-center text-white bg-blue-700/80 rounded-full hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
...
function create_loader() {
    ...
    svg_item.className = "w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
...
async function fetch_movies(search_string, signal) {
    ...
    return {
        ...
        plot: movie["Plot"],
        poster: movie["Poster"],
        rating: movie["Ratings"]?.filter(rating => rating["Source"] == "Internet Movie Database")[0]["Value"],
